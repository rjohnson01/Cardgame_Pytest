...
def build_play_deck():
    ...
    for cards in deck:                                                              #Loop through all the card objects in the deck
        if cards["type"] == "hero":                                                 #Create Hero card object using the current card values
            ...
            build_deck.append(Spellcard(cards["type"], cards["name"], cards["desc"], cards["health"], cards["str"]))
...
def card_actions(attacking_card, defending_card):
    if attacking_card.type == "hero":
        #if defending_card.type is not "spell":
...
if __name__ == "__main__":
    ...
    while True:
        ...
        player_card = player.cards_in_play[random.randrange(0, len(player.cards_in_play))]
        ai_player_card = ai_player.cards_in_play[random.randrange(0, len(player.cards_in_play))]


        card_actions(player_card, ai_player_card)
        if ai_player.cards_in_play[0].health <= 0:
            del ai_player.cards_in_play[0]

        card_actions(ai_player_card, player_card)
