...
# testing that the game intro is displaying
...
def test_draw_new_card(test_play_deck):
    test_deck = test_play_deck
    drawn_card = draw_new_card(test_deck)

    print("Drawn card is " + drawn_card.name)

    assert drawn_card.name != ""

# testing that player_card_selection is pulling random cards
""" def test_player_card_selection(test_play_deck):
    test_player = Player()
    test_player_hand = fill_players_hand(test_play_deck)
    test_player.hand = test_player_hand
    random_card = test_player.play_random_card()

    #print("Random card drawn is " + random_card.name)
    
    assert random_card.name != "" """
